...
export default function ApplyPage() {
  ...
  return (
    <div className="page-shell max-w-4xl space-y-5">
      <div>
        <div className="page-kicker">{t("apply.kicker")}</div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder={t("apply.incomePlaceholder")}
              className="field-input"
            />
          </div>
        </div>
        ...
        <div className="ui-card p-5 space-y-4">
          <div>
            ...
            {result.fpl_percent != null && (
              <div className="text-sm text-slate-600">{t("apply.estimatedIncome", { percent: result.fpl_percent })}</div>
            )}
            {verdictText && (
          ...
          <div>
            ...
            <a href={`tel:${result.hospital.phone}`} className="mt-2 inline-block text-brand-600 underline text-sm">
              {t("apply.call", { phone: result.hospital.phone })}
            </a>
          </div>
        </div>
      )}
        ...
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-5 shadow-sm space-y-2">
          ...
          {!eligibility ? (
            <button onClick={checkMedicaid} className="rounded-lg bg-amber-700 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-amber-800">
              {t("apply.checkMedicaid")}
            </button>
          ) : (
            <div className="text-sm space-y-2">
              <div>
                {t("apply.likelyEligible")}: <strong>{eligibility.eligible ? t("common.yes") : t("common.unclear")}</strong>
                {eligibility.fpl_percent != null && ` (${t("apply.medicaidThreshold", { percent: eligibility.fpl_percent })})`}
              </div>
              <div>{t("apply.program")}: {t("apply.emergencyMedicalProgram")}</div>
              <div>
              ...
              <ul className="list-disc ms-5 text-xs text-slate-600">
                {eligibility.required_documents.map((_, i) => <li key={i}>{t(`apply.medicaidDoc${i + 1}`)}</li>)}
              </ul>
            </div>
          )}
        </div>
