...
export default function CarePage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/care", {
      ...
      .catch((e) => {
        setError(typeof e === "string" ? e : t("care.loadError"));
        setLoading(false);
      });
  }, [lang]);
  ...
  return (
    <div className="page-shell space-y-5">
      <div>
        <div className="page-kicker">{t("care.kicker")}</div>
        <h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-950">{t("care.header")}</h1>
        <div className="mt-2 text-sm text-slate-600">
          {t("care.zipResults", { zip, count: data?.results.length ?? 0 })}
        </div>
      </div>

      {emergency && <EmergencyBanner match={emergency} onDismiss={() => setEmergency(null)} />}

      {loading && <div className="ui-card p-4 text-slate-500">{t("common.loading")}</div>}
