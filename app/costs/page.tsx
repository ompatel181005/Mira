...
export default function CostsPage() {
  ...
  async function search() {
    ...
    try {
      const r = await fetch("/api/costs/medication", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ name, zip, language: lang }),
      });
  ...
  return (
    <div className="page-shell max-w-4xl space-y-5">
      <div>
        <div className="page-kicker">{t("costs.kicker")}</div>
      ...
      {med && (
        <div className="ui-card p-5 space-y-3">
            ...
            <div>
              ...
              {med.generic && (
                <div className="text-sm text-slate-600">
                  {t("costs.generic")}: <span className="font-medium">{med.generic}</span>
                </div>
              )}
            </div>
            {med.purpose && (
              <div className="text-xs text-slate-500 max-w-md">{t("costs.usedFor")}: {med.purpose.slice(0, 200)}</div>
            )}
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-left text-xs uppercase text-slate-500">
                <tr>
                  <th className="rounded-l-lg px-3 py-2">{t("costs.source")}</th>
                  <th className="px-3">{t("costs.price")}</th>
                  <th className="px-3">{t("costs.notes")}</th>
              ...
              <tbody>
                {med.sources.map((s, i) => (
                  <tr key={i} className="border-b border-slate-100 last:border-0">
                    ...
                    <td className="px-3 text-slate-600">{s.note_key ? t(`costs.api.${s.note_key}`) : s.notes}</td>
                    <td className="px-3">
                      {s.url && s.action_label && (
                        <a href={s.url} target="_blank" rel="noreferrer" className="text-brand-600 underline text-sm">
                          {s.action_key ? t(`costs.api.action.${s.action_key}`) : s.action_label}
                        </a>
                      )}
                    </td>
                  </tr>
          ...
          {med.paps.length > 0 && (
            <div className="mt-3">
              <h3 className="font-semibold">{t("costs.manufacturerAssistance")}</h3>
          ...
          {charities.map((c, i) => (
            <li key={i} className="py-3 flex flex-wrap gap-2 items-baseline">
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-slate-500 mt-0.5">{t("common.languages")}: {(c.languages || []).join(", ")}</div>
              </div>
              <div className="flex items-center gap-2 text-sm">
                {c.distance_miles != null && (
                  <span className="text-xs text-slate-500">{t("common.mi", { count: c.distance_miles })}</span>
                )}
                <a href={`tel:${c.phone}`} className="text-brand-600 underline">{c.phone}</a>
                <a href={c.url} target="_blank" rel="noreferrer" className="text-brand-600 underline">{t("common.site")}</a>
              </div>
            </li>
          ))}
        </ul>
