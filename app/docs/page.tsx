...
export default function DocsPage() {
  ...
  async function onUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (!r.ok) {
        setError(j.error || t("docs.uploadFailed"));
      } else {
        setDocs((d) => [...d, j.doc]);
  ...
  async function sendChat() {
    ...
    try {
      ...
      const j = await r.json();
      setChatMessages((m) => [...m, { role: "assistant", content: j.reply || t("docs.noResponse") }]);
    } finally {
  ...
  return (
    <div className="page-shell max-w-4xl space-y-5">
      <div>
        <div className="page-kicker">{t("docs.kicker")}</div>
      ...
      <label className="block cursor-pointer rounded-lg border-2 border-dashed border-teal-200 bg-white/85 p-8 text-center shadow-sm transition hover:border-teal-400 hover:bg-teal-50/60">
        ...
        {loading && <div className="text-sm text-brand-600 mt-2">{t("docs.processing")}</div>}
      </label>
        ...
        <div key={i} className="ui-card p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="soft-pill border-slate-200 bg-slate-100 text-slate-600">
              {d.extracted?.doc_type || t("docs.document")}
            </span>
          </div>
          {d.summary && (
            <div>
          ...
          {(d.extracted?.bill_total != null || d.extracted?.patient_responsibility != null) && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm">
              {d.extracted.patient_responsibility != null && (
                <div>{t("docs.youOwe")}: <strong>${d.extracted.patient_responsibility}</strong></div>
              )}
              {d.extracted.due_date && <div>{t("docs.due")}: {d.extracted.due_date}</div>}
              <a href="/apply" className="text-brand-600 underline text-sm">{t("docs.getCharityHelp")}</a>
            </div>
          )}
        </div>
      ))}
          ...
          <div className="space-y-2 max-h-60 overflow-auto">
            {chatMessages.map((m, i) => (
              <div key={i} className={`text-sm ${m.role === "user" ? "text-slate-900" : "text-slate-700 bg-slate-50 p-2 rounded"}`}>
                <span className="font-medium me-1">{m.role === "user" ? t("docs.you") : "MIRA"}:</span>
                {m.content}
              </div>
            ))}
