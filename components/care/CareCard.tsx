    ...
    <div className="ui-card p-4 transition hover:border-teal-200 hover:shadow-md">
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <div className="text-xs text-slate-500 mt-1">{t("care.milesAway", { count: r.distance_miles })}</div>
        </div>
      </div>
      ...
      {open && (
        <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
          <ul className="list-disc ms-5 space-y-1">
            ...
            {r.sliding_scale && <li>{t("care.reasonSlidingScale")}</li>}
            {r.accepts_uninsured && <li>{t("care.reasonUninsured")}</li>}
            {r.languages.length > 0 && (
              <li>{t("common.languages")}: {r.languages.join(", ")}</li>
            )}
            {(r.services || []).slice(0, 6).length > 0 && (
              <li>{t("care.services")}: {(r.services || []).slice(0, 6).join(", ")}</li>
            )}
          </ul>
        </div>
      )}
